...
function UnifiedVisibilitySection() {
  ...
  // Helper function to get consistent colors for brands
  const getColorForBrand = (brandName: string, index: number) => {
    const colors = [
      '#3B82F6', // blue
      '#EF4444', // red
      '#8B5CF6', // purple
  ...
  // Get real data from analytics context
  const visibilityData = data.visibility
  const brandMetrics = visibilityData?.brandMetrics || []
  
  // Transform brand metrics to chart data format
  const realChartData = brandMetrics.slice(0, 5).map((brand, index) => ({
    name: brand.brandName,
    score: parseFloat(formatToTwoDecimals(brand.visibilityScore || 0)),
    color: getColorForBrand(brand.brandName, index),
    comparisonScore: parseFloat(formatToTwoDecimals((brand.visibilityScore || 0) * 0.9)) // Mock comparison data for now
  }))

  // Transform brand metrics to rankings format
  const realRankings = brandMetrics.map((brand, index) => ({
  ...
  const displayChartData = realChartData.length > 0 ? realChartData : chartData
  const displayRankings = realRankings.length > 0 ? realRankings : rankings
  
  // Set active platform to first item if not set
  if (!activePlatform && displayChartData.length > 0) {
    setActivePlatform(displayChartData[0].name)
  }

  // Get user's brand score (rank 1 or first brand)
  const userBrand = brandMetrics.find(b => b.visibilityRank === 1) || brandMetrics[0]
  const userBrandScore = userBrand?.visibilityScore || 0
          ...
          <div className="space-y-6 relative">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-baseline gap-3">
                <div className="metric text-2xl text-foreground">{userBrandScore}%</div>
                {showComparison && (
                ...
                <>
                  ...
                  <div className="absolute left-2 top-3 bottom-3 flex flex-col justify-between caption text-muted-foreground">
                    {displayChartData.length > 0 && (
                      <>
                        <span>{formatPercentage(Math.max(...displayChartData.map(d => d.score)))}</span>
                        <span>{formatPercentage(Math.max(...displayChartData.map(d => d.score)) * 0.8)}</span>
                        <span>{formatPercentage(Math.max(...displayChartData.map(d => d.score)) * 0.6)}</span>
                        <span>{formatPercentage(Math.max(...displayChartData.map(d => d.score)) * 0.4)}</span>
                        <span>{formatPercentage(Math.max(...displayChartData.map(d => d.score)) * 0.2)}</span>
                        <span>0%</span>
                      </>
                    )}
                  </div>
                    ...
                    {displayChartData.map((bar) => (
                      <div 
                        ...
                        onMouseEnter={(e) => {
                          ...
                          setHoveredBar({
                            name: bar.name,
                            score: formatPercentage(bar.score),
                      ...
                      >
                        ...
                        <div className="caption text-foreground text-center">
                          {showComparison ? (
                            <div className="space-y-1">
                              <div className="text-foreground">{formatPercentage(bar.score)}</div>
                              <div className="text-muted-foreground text-[10px]">
                                {bar.comparisonScore}%
                              </div>
                            </div>
                          ) : (
                            formatPercentage(bar.score)
                          )}
                        </div>
                        ...
                        <div className="flex items-end gap-1">
                          ...
                          <div 
                            ...
                            style={{
                              height: `${(bar.score / Math.max(...displayChartData.map(d => d.score))) * 120}px`,
                              minHeight: '4px',
                          ...
                          {showComparison && (
                            <div
                              ...
                              style={{
                                height: `${(bar.comparisonScore / Math.max(...displayChartData.map(d => d.score))) * 120}px`,
                                minHeight: '2px',
                                backgroundColor: bar.color,
                        ...
                        <Label
                          content={({ viewBox }) => {
                            if (viewBox && "cx" in viewBox && "cy" in viewBox) {
                              const activeData = displayChartData[activeIndex] || displayChartData[0]
                              return (
                                <text
                                  x={viewBox.cx}
                                  y={viewBox.cy}
                                ...
                                >
                                  ...
                                  >
                                    {formatPercentage(activeData?.score || 0)}
                                  </tspan>
                                  <tspan
                                    x={viewBox.cx}
                                    y={(viewBox.cy || 0) + 16}
                    ...
                    {showComparison && (
                      <div className="flex justify-between items-center text-xs">
                        ...
                        <span className="text-gray-400">
                          {(() => {
                            const platform = displayChartData.find(p => p.name === hoveredBar.name)
